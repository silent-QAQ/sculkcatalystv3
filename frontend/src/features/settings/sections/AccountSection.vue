...
import {
  ...
  Link2, LockKeyhole, LogIn, LogOut, MailPlus, MessageSquareText, Monitor, Plus, RadioTower, RefreshCw, Save, Server,
  Settings2, ShieldCheck, SquareTerminal, Trash2, Upload, UserPlus, Users, XCircle,
} from 'lucide-vue-next'
import {
  CLOUD_SESSION_EXPIRED_EVENT, CloudApiError, cloudRequest, cloudSession, setCloudSession,
} from '../../cloud/client'
import {
  applyCloudWorkspace, cloudPrompts, cloudSkillLinks, isCloudWorkspace,
  saveCloudPrompts, saveCloudSkillLinks, snapshotCloudWorkspace,
} from '../../cloud/workspace'
import type {
  ApiTokenCreated, ApiTokenItem, AuthResponse, CloudApproval, CloudDevice, CloudProfile,
  CloudCredential, CloudStatus, CloudTeam, CloudWorkspacePayload, DeploymentCapability, Invitation, RelayProvider, SyncedSettings,
  TeamMember, UsageSummary,
} from '../../cloud/types'
import type { UiSettings } from '../types'
import { aiSettings, applyCloudUi, uiSettings } from '../store'
import ServerWorkspaceView from '../../cloud/ServerWorkspaceView.vue'
import AgentDevicesView from '../../cloud/AgentDevicesView.vue'
import AgentTasksView from '../../cloud/AgentTasksView.vue'
import type { ServerTemplate } from '../../portable/server-manifest'

const TerminalSessionsView = defineAsyncComponent(() => import('../../cloud/TerminalSessionsView.vue'))
const CloudConversationsView = defineAsyncComponent(() => import('../../cloud/CloudConversationsView.vue'))
...
const authForm = ref({ email: '', password: '', nickname: '', device_name: isWeb.value ? 'Sculk Cloud 网页版' : 'Sculk 桌面工作台' })
...
const credentialForm = ref({ name: '', base_url: 'https://api.openai.com/v1', api_key: '' })
const promptForm = ref({ title: '', content: '' })
const skillLinkForm = ref({ name: '', url: '' })
...
const availableTabs: { key: CloudTab; label: string; icon: typeof Cloud; feature?: string; badge?: () => number }[] = [
  { key: 'overview', label: '账号概览', icon: Cloud },
  { key: 'agents', label: '主机代理', icon: RadioTower, feature: 'agents' },
  { key: 'tasks', label: '远程任务', icon: SquareTerminal, feature: 'agent-tasks' },
  { key: 'terminal', label: '终端', icon: Monitor, feature: 'terminal-sessions' },
  { key: 'conversations', label: '对话', icon: MessageSquareText, feature: 'cloud-conversations' },
  { key: 'workspace', label: '开服工作区', icon: Server, feature: 'sync' },
  { key: 'api', label: 'API 中转', icon: Activity, feature: 'api-relay' },
  { key: 'team', label: '我的团队', icon: Users, feature: 'teams' },
  { key: 'approvals', label: '远程审批', icon: ShieldCheck, feature: 'approvals', badge: () => pendingApprovals.value },
  { key: 'deployments', label: '云部署', icon: Server },
]
const tabs = computed(() => availableTabs.filter(item => {
  if (!isWeb.value) return true
  if (item.key === 'deployments') return false
  return !item.feature || status.value?.features.includes(item.feature)
}))
...
async function initialize() {
  ...
  try {
    ...
    if (!isWeb.value) capability.value = await cloudRequest<DeploymentCapability>('/api/cloud/deployments/capability', {}, false)
  } catch (error) {
...
async function loadWorkspace() {
  ...
  const results = await Promise.allSettled([
    ...
    cloudRequest<CloudCredential[]>('/api/cloud/credentials'),
  ])
  if (results[0].status === 'fulfilled') devices.value = results[0].value
  if (results[1].status === 'fulfilled') await applySyncedWorkspace(results[1].value, true)
  ...
  if (results[6].status === 'fulfilled') credentials.value = results[6].value
...
async function authenticate() {
  ...
  try {
    ...
    await loadWorkspace()
    flash(authMode.value === 'login' ? '已登录并自动同步云端配置' : isWeb.value ? '云账号已创建并完成初始化' : '云账号已创建并同步本机配置')
  } catch (error) {
    flash(errorText(error), 'error')
  } finally {
    busy.value = ''
...
function localWorkspace(overrideUi?: UiSettings): CloudWorkspacePayload {
  const payload = snapshotCloudWorkspace(overrideUi || uiSettings.value, aiSettings.value)
  return { ...(synced.value?.payload || {}), ...payload }
}

async function syncWorkspace(successMessage: string) {
  if (!synced.value) return
  const payload = localWorkspace()
  synced.value = await cloudRequest<SyncedSettings>('/api/cloud/sync/settings', {
    method: 'PUT',
    body: JSON.stringify({ base_version: synced.value.version, payload }),
  })
  applyCloudWorkspace(payload)
  flash(successMessage.replace('{version}', String(synced.value.version)))
}

async function applySyncedWorkspace(next: SyncedSettings, seedWhenEmpty = false) {
  synced.value = next
  if (isCloudWorkspace(next.payload)) {
    const cloudUi = applyCloudWorkspace(next.payload)
    if (cloudUi) applyCloudUi(cloudUi)
    return
  }

  const legacyUi = next.payload.ui as UiSettings | undefined
  if (legacyUi) applyCloudUi(legacyUi)
  if (!seedWhenEmpty && !legacyUi) throw new Error('云端还没有可应用的工作台设置')

  const migrated = localWorkspace(legacyUi)
  try {
    synced.value = await cloudRequest<SyncedSettings>('/api/cloud/sync/settings', {
      method: 'PUT',
      body: JSON.stringify({ base_version: next.version, payload: migrated }),
    })
    applyCloudWorkspace(migrated)
  } catch (error) {
    if (!(error instanceof CloudApiError) || error.status !== 409) throw error
    const latest = await cloudRequest<SyncedSettings>('/api/cloud/sync/settings')
    synced.value = latest
    if (isCloudWorkspace(latest.payload)) {
      const cloudUi = applyCloudWorkspace(latest.payload)
      if (cloudUi) applyCloudUi(cloudUi)
    }
  }
}
...
async function pushSettings() {
  ...
  busy.value = 'sync-up'
  try {
    await syncWorkspace('设置、提示词与 Skill 链接已同步（版本 {version}）')
  } catch (error) { flash(errorText(error), 'error') } finally { busy.value = '' }
}
...
async function pullSettings() {
  ...
  try {
    synced.value = await cloudRequest<SyncedSettings>('/api/cloud/sync/settings')
    await applySyncedWorkspace(synced.value)
...
async function syncServerTemplates() {
  ...
  try {
    await syncWorkspace('开服参数模板已同步（版本 {version}）')
  } catch (error) {
    flash(`模板已保存在当前浏览器，但云端同步失败：${errorText(error)}`, 'error')
  } finally {
    busy.value = ''
  }
}
...
async function addPrompt() {
  ...
  if (!title || !content) return flash('请填写提示词名称和内容', 'error')
  saveCloudPrompts([...cloudPrompts.value, { id: localId('prompt'), title, content }])
  promptForm.value = { title: '', content: '' }
  await pushSettings()
}

async function removePrompt(id: string) {
  saveCloudPrompts(cloudPrompts.value.filter(item => item.id !== id))
  await pushSettings()
}

async function addSkillLink() {
  const name = skillLinkForm.value.name.trim()
  const url = skillLinkForm.value.url.trim()
  try {
    const parsed = new URL(url)
    if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error()
  } catch {
    return flash('Skill 链接需要是有效的 HTTP(S) 地址', 'error')
  }
  if (!name) return flash('请填写 Skill 名称', 'error')
  saveCloudSkillLinks([...cloudSkillLinks.value, { id: localId('skill'), name, url, enabled: true }])
  skillLinkForm.value = { name: '', url: '' }
  await pushSettings()
}

async function removeSkillLink(id: string) {
  saveCloudSkillLinks(cloudSkillLinks.value.filter(item => item.id !== id))
  await pushSettings()
}

async function saveCredential() {
  busy.value = 'credential'
  try {
    const item = await cloudRequest<CloudCredential>('/api/cloud/credentials', {
      method: 'POST', body: JSON.stringify(credentialForm.value),
    })
    credentials.value = [item, ...credentials.value.filter(existing => existing.id !== item.id)]
    credentialForm.value.api_key = ''
    flash('API 凭据已加密保存')
  } catch (error) { flash(errorText(error), 'error') } finally { busy.value = '' }
}

async function deleteCredential(id: string) {
  busy.value = `credential:${id}`
  try {
    await cloudRequest<void>(`/api/cloud/credentials/${id}`, { method: 'DELETE' })
...
async function createApproval() {
  ...
  try {
    const item = await cloudRequest<CloudApproval>('/api/cloud/approvals', {
      method: 'POST',
      body: JSON.stringify({ team_id: activeTeamId.value, ...approvalForm.value, payload: { source: isWeb.value ? 'sculk-cloud-web' : 'sculk-workbench' } }),
    })
...
onMounted(() => {
  window.addEventListener(CLOUD_SESSION_EXPIRED_EVENT, handleSessionExpired)
  void initialize()
})
onUnmounted(() => window.removeEventListener(CLOUD_SESSION_EXPIRED_EVENT, handleSessionExpired))
</script>
...
<template>
  <div class="cloud-console">
    <header class="cloud-console-head">
      ...
      <div><p>Sculk Cloud</p><h2>{{ profile ? profile.nickname : isWeb ? '云端协作控制台' : '连接你的工作台' }}</h2></div>
      <span class="cloud-state" :class="{online:status?.available}"><i/>{{ status?.available ? '服务已连接' : isWeb ? '服务不可用' : '服务未配置' }}</span>
    ...
    <section v-if="busy === 'initialize' && !status" class="cloud-unavailable public">
      <div><RefreshCw class="s-spin"/><h3>正在连接 Sculk Cloud</h3><p>正在确认云服务状态与账号会话。</p></div>
    </section>

    <section v-else-if="!status?.available && !busy" class="cloud-unavailable" :class="{ public: isWeb }">
      <div><CircleDashed/><h3>{{ isWeb ? '云服务暂时不可用' : '云服务等待配置' }}</h3><p>{{ isWeb ? '当前无法连接 Sculk Cloud，请稍后重试。' : status?.message || '无法读取 Sculk Cloud 状态' }}</p><button v-if="isWeb" class="cloud-primary compact" @click="initialize"><RefreshCw/>重新连接</button></div>
      <dl v-if="!isWeb"><div><dt>PostgreSQL</dt><dd>DATABASE_URL</dd></div><div><dt>Redis</dt><dd>REDIS_URL</dd></div><div><dt>密钥</dt><dd>SCULK_MASTER_KEY</dd></div></dl>
    </section>

    <section v-else-if="!profile" class="cloud-auth">
      <div class="cloud-auth-copy"><LockKeyhole/><h3>{{ authMode==='login' ? '登录云账号' : '创建云账号' }}</h3><p>登录后会自动同步界面设置、提示词、Skill 链接和非敏感模型配置。</p><ul><li><Check/>登录自动同步</li><li><Check/>版本冲突保护</li><li><Check/>API 凭据加密隔离</li></ul></div>
    ...
    <template v-else>
      ...
      <section v-if="tab==='overview'" class="cloud-view cloud-overview">
        ...
        <article class="cloud-panel sync-panel">
          ...
          <div class="sync-version"><span>{{ isWeb ? '当前浏览器' : '本机' }}</span><i/><b>设置快照</b><i/><span>云端 v{{ synced?.version || 1 }}</span></div>
          <div class="cloud-actions"><button :disabled="!!busy" @click="pullSettings"><Download/>应用云端</button><button class="accent" :disabled="!!busy" @click="pushSettings"><Upload/>{{ isWeb ? '同步浏览器' : '同步本机' }}</button></div>
        ...
        <article class="cloud-panel workspace-panel">
          <header><div><h3>云端工作区</h3><p>提示词与 Skill 链接会随账号自动同步</p></div><Cloud/></header>
          <div class="workspace-columns">
            <section>
              <h4><MessageSquareText/>快捷提示词 <span>{{ cloudPrompts.length }}</span></h4>
              <form @submit.prevent="addPrompt"><input v-model="promptForm.title" maxlength="32" placeholder="名称" required/><input v-model="promptForm.content" maxlength="500" placeholder="提示词内容" required/><button class="cloud-primary compact" :disabled="!!busy"><Plus/>添加</button></form>
              <div class="workspace-items"><div v-for="item in cloudPrompts" :key="item.id"><p><b>{{ item.title }}</b><small>{{ item.content }}</small></p><button class="cloud-icon-btn danger" title="删除提示词" :disabled="!!busy" @click="removePrompt(item.id)"><Trash2/></button></div><div v-if="!cloudPrompts.length" class="workspace-empty">暂无快捷提示词</div></div>
            </section>
            <section>
              <h4><Link2/>Skill 链接 <span>{{ cloudSkillLinks.length }}</span></h4>
              <form @submit.prevent="addSkillLink"><input v-model="skillLinkForm.name" maxlength="64" placeholder="Skill 名称" required/><input v-model="skillLinkForm.url" type="url" placeholder="https://github.com/…" required/><button class="cloud-primary compact" :disabled="!!busy"><Plus/>添加</button></form>
              <div class="workspace-items"><div v-for="item in cloudSkillLinks" :key="item.id"><p><b>{{ item.name }}</b><a :href="item.url" target="_blank" rel="noopener noreferrer">{{ item.url }}</a></p><button class="cloud-icon-btn danger" title="删除 Skill 链接" :disabled="!!busy" @click="removeSkillLink(item.id)"><Trash2/></button></div><div v-if="!cloudSkillLinks.length" class="workspace-empty">暂无 Skill 链接</div></div>
            </section>
          </div>
        </article>
      </section>

      <section v-else-if="tab==='workspace'" class="cloud-view">
        <ServerWorkspaceView
          :context="props.context"
          :busy="busy === 'workspace'"
          @changed="syncServerTemplates"
          @apply="template => emit('applyServerTemplate', template)"
        />
      </section>

      <section v-else-if="tab==='agents'" class="cloud-view">
        <AgentDevicesView/>
      </section>

      <section v-else-if="tab==='tasks'" class="cloud-view">
        <AgentTasksView/>
      </section>

      <section v-else-if="tab==='terminal'" class="cloud-view">
        <TerminalSessionsView/>
      </section>

      <section v-else-if="tab==='conversations'" class="cloud-view">
        <CloudConversationsView/>
      </section>
      ...
      <section v-else-if="tab==='api'" class="cloud-view">
        ...
        <article class="cloud-panel credential-panel">
          <header><div><h3>个人 API 凭据</h3><p>AES-256-GCM 加密保存 · SHA-256 指纹去重 · 页面仅显示掩码</p></div><LockKeyhole/></header>
          <form class="credential-form" @submit.prevent="saveCredential"><label>名称<input v-model="credentialForm.name" maxlength="64" placeholder="OpenAI 主账号" required/></label><label>Base URL<input v-model="credentialForm.base_url" type="url" placeholder="https://api.openai.com/v1" required/></label><label>API Key<input v-model="credentialForm.api_key" type="password" autocomplete="off" placeholder="sk-…" minlength="8" required/></label><button class="cloud-primary compact" :disabled="busy==='credential'"><Save/>加密保存</button></form>
          <div class="credential-list"><div v-for="item in credentials" :key="item.id"><span><KeyRound/></span><p><b>{{ item.name }}</b><small>{{ item.base_url }}</small></p><p class="credential-mask"><code>{{ item.api_key_masked }}</code><small>指纹 {{ item.fingerprint }} · {{ formatDate(item.updated_at) }}</small></p><button class="cloud-icon-btn danger" title="删除 API 凭据" :disabled="busy===`credential:${item.id}`" @click="deleteCredential(item.id)"><Trash2/></button></div><div v-if="!credentials.length" class="cloud-empty"><LockKeyhole/>尚未保存个人 API 凭据</div></div>
        </article>
