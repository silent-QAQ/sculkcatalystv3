...
import { Activity, AlertTriangle, Archive, Bot, Box, BrainCircuit, ChevronDown, ChevronRight, ChevronUp, CircleStop, Copy, CornerDownRight, Cpu, Database, Download, FileCode2, FileUp, Files, Folder, FolderTree, Gauge, GitBranch, LayoutDashboard, ListPlus, MapPin, MessageSquareText, MoreHorizontal, PanelLeftClose, Paperclip, Pencil, Play, PlugZap, Plus, RefreshCw, RotateCcw, RotateCw, Search, Send, Server, Settings, ShieldCheck, Sparkles, SquareTerminal, Trash2, Users, Vote, X, Coffee, HardDrive, Check, LoaderCircle } from 'lucide-vue-next'
...
import type { AiSettingsView, ModelBinding, ReasoningEffort, ReviewMode } from './features/settings/types'
...
import { TASK_STATUS_LABELS } from './features/automation/types'
...
interface ServerItem { id:string; name:string; kind?:WorkspaceKind; core:string; version:string; status:Status; operation_state?:ServerOperationState; core_ready?:boolean; last_error?:string|null; players:string; memory:number; memory_gb?:number; cpu:number; port:number; task:string; location?:string }
interface Message { id:string; role:'assistant'|'user'; content:string; time:string; actions?:string[]; task_id?:string; streaming?:boolean; fallback?:boolean; warning?:string; error?:boolean; interrupted?:boolean; retryContent?:string }
...
const servers = ref<ServerItem[]>([]), selectedId = ref(''), collapsed = ref(false), tab = ref<Tab>('overview'), activeFile = ref('')
...
const emptyServer:ServerItem={id:'',name:'未选择工作区',kind:'server',core:'',version:'',status:'stopped',operation_state:'idle',core_ready:false,last_error:null,players:'- / -',memory:0,memory_gb:8,cpu:0,port:0,task:'请创建或选择工作区',location:'local'}
...
const showAgentMenu = ref(false), showModelMenu = ref(false), showReasoningMenu = ref(false), showReviewMenu = ref(false)
...
async function pickChatModel(binding:ModelBinding|null){
  ...
  try{const summary=await persistConversationExecution(binding,chatAgentOverride.value,chatReasoningEffort.value);if(selectedId.value!==workspaceId||selectedConversationId.value!==summary.id)return;chatModelOverride.value=binding;flash(binding?'当前对话将使用 '+chatModelLabel.value:'当前对话已恢复按情景绑定自动选择模型')}
  catch(error){flash('模型选择保存失败：'+String(error))}
}
async function pickChatAgent(agentId:string|null):Promise<boolean>{
  ...
  try{
    ...
    const values=chosen?(aiSettings.value?.detected_agents?.find(item=>item.kind===chosen.kind)?.capabilities.reasoning_effort.values??[]):[]
    const nextEffort=chosen&&(chosen.transport??'acp')==='cli'&&chatReasoningEffort.value&&values.length&&!values.includes(chatReasoningEffort.value)?null:chatReasoningEffort.value
...
async function pickReasoningEffort(effort:ReasoningEffort|null){
  const workspaceId=selectedId.value;showReasoningMenu.value=false
  try{const summary=await persistConversationExecution(chatModelOverride.value,chatAgentOverride.value,effort);if(selectedId.value!==workspaceId||selectedConversationId.value!==summary.id)return;chatReasoningEffort.value=effort;flash(effort?`当前对话思考强度已设为${REASONING_EFFORTS.find(item=>item.key===effort)?.label??effort}`:'当前对话已恢复自动思考强度')}
  catch(error){flash('思考强度保存失败：'+String(error))}
}
...
function closeMenus(event:MouseEvent){
  if(!(event.target as HTMLElement).closest('.composer-menu-anchor')){showAgentMenu.value=false;showModelMenu.value=false;showReasoningMenu.value=false;showReviewMenu.value=false;showContextMenu.value=false}
  if(!(event.target as HTMLElement).closest('.file-context-menu'))fileContextMenu.value=null
}
...
async function executeDeleteServer(){
  ...
  try{
    ...
    for(const [conversationId,items] of Object.entries(chatQueues.value)){if(items.some(item=>item.serverId===target.id))setQueue(conversationId,[])}
    for(const run of Object.values(chatRuns.value)){if(run.serverId===target.id)chatControllers.get(run.conversationId)?.abort()}
  ...
  }catch(error){flash('删除失败：'+String(error))}finally{busy.value=false}
}
...
async function createFromContext(kind:'file'|'folder',entry?:FileEntry){
  ...
  showNewFile.value=kind==='file';showNewFolder.value=kind==='folder'
...
async function submitFileAction(){
  ...
  try{
    if(dialog.kind==='rename'){
      ...
      await api('/api/servers/'+selectedId.value+'/file/rename',{method:'POST',body:JSON.stringify({path:dialog.entry.path,new_path:newPath})})
      if(activeFile.value===dialog.entry.path||activeFile.value.startsWith(dialog.entry.path+'/'))activeFile.value=newPath+activeFile.value.slice(dialog.entry.path.length)
      flash('已重命名为 '+name)
    }else{
...
async function selectServer(id:string){
  ...
  selectedId.value=id;surface.value='control';tab.value=project?'files':'overview';activeFile.value='';mirrorPanel.value=false;previewCandidates.value=[];downloadStatus.value=null;stopDownloadPolling()
...
async function loadDashboardOnce(loadRelated=true){
  ...
  try{
    ...
    servers.value=Array.isArray(data.servers)?data.servers:[];tasks.value=Array.isArray(data.tasks)?data.tasks:[]
...
onUnmounted(()=>{document.removeEventListener('click',closeMenus);if(refreshTimer)window.clearInterval(refreshTimer);for(const controller of chatControllers.values())controller.abort();chatControllers.clear();disconnectLogStream();stopDownloadPolling()})
</script>
  ...
  <main class="app" :class="{collapsed,'mirror-mode':surface==='mirror'||surface==='settings'}">
    ...
    <section v-if="surface!=='mirror'&&surface!=='settings'" class="chat-panel">
      ...
      <div ref="scroller" class="chat-scroll">
        ...
        <section v-else-if="server.status==='planning'" class="mission planning-mission"><div><span class="agent"><BrainCircuit :size="20"/></span><p><small>智能创建 · 规划阶段</small><b>通过对话确定核心、版本与部署方案</b></p><em><i/>未创建文件</em></div><footer><span>先描述玩法、人数与版本偏好</span><span>方案确认后再创建工作区</span></footer></section>
      ...
      <div v-if="selectedId" class="compose-wrap">
        <div class="prompts"><button v-for="prompt in cloudPrompts" :key="prompt.id" @click="send(prompt.content)"><MessageSquareText/>{{ prompt.title }}</button></div>
        ...
        <span class="composer-menu-anchor">
          <button :class="{active:showContextMenu}" title="查看发送上下文" @click.stop="showContextMenu=!showContextMenu;showAgentMenu=false;showModelMenu=false;showReasoningMenu=false;showReviewMenu=false"><Paperclip/></button>
        ...
        <span class="composer-menu-anchor">
          <button class="model" :class="{active:showAgentMenu}" @click.stop="showAgentMenu=!showAgentMenu;showModelMenu=false;showReasoningMenu=false;showReviewMenu=false"><Sparkles/>{{chatAgentLabel}}<ChevronDown/></button>
          <div v-if="showAgentMenu" class="composer-menu">
            ...
            <template v-if="agentMenuItems.length">
              <small class="group">已接入的智能体</small>
              <button v-for="agent in agentMenuItems" :key="agent.id" :class="{picked:activeAgentId===agent.id}" @click="pickChatAgent(agent.id)"><span><b>{{agent.name}}</b><em>{{agent.kind}} · {{(agent.transport??'acp')==='cli'?'原生 CLI':'ACP 协议'}}</em></span><Check v-if="activeAgentId===agent.id"/></button>
            </template>
            <small v-else class="empty-hint">尚未接入外部智能体，可到「设置」自动检测 CLI 或手动接入</small>
          </div>
        </span>
        <span v-if="!activeAgentId" class="composer-menu-anchor">
          <button class="model" :class="{active:showModelMenu}" @click.stop="showModelMenu=!showModelMenu;showAgentMenu=false;showReasoningMenu=false;showReviewMenu=false"><Cpu/>{{chatModelLabel}}<ChevronDown/></button>
        ...
        <span v-if="reasoningSupported" class="composer-menu-anchor">
          <button class="model" :class="{active:showReasoningMenu}" @click.stop="showReasoningMenu=!showReasoningMenu;showAgentMenu=false;showModelMenu=false;showReviewMenu=false"><BrainCircuit/>{{reasoningLabel}}<ChevronDown/></button>
          <div v-if="showReasoningMenu" class="composer-menu">
            <small>当前对话的思考强度</small>
            <button :class="{picked:!chatReasoningEffort}" @click="pickReasoningEffort(null)"><span><b>自动</b><em>跟随模型、Agent 或系统默认值</em></span><Check v-if="!chatReasoningEffort"/></button>
            <button v-for="effort in reasoningMenuItems" :key="effort.key" :class="{picked:chatReasoningEffort===effort.key}" @click="pickReasoningEffort(effort.key)"><span><b>{{effort.label}}</b><em>{{effort.hint}}</em></span><Check v-if="chatReasoningEffort===effort.key"/></button>
          </div>
        </span>
        <span class="composer-menu-anchor">
          <button class="model review" :class="{active:showReviewMenu,warn:reviewMode==='full'}" @click.stop="showReviewMenu=!showReviewMenu;showAgentMenu=false;showModelMenu=false;showReasoningMenu=false"><ShieldCheck/>{{reviewModeLabel}}<ChevronDown/></button>
    ...
    <div v-if="fileActionDialog" class="modal-backdrop" @click.self="fileActionDialog=null">
      <section class="action-modal">
        ...
        <main v-if="fileActionDialog.kind==='rename'"><div class="field"><label>新名称</label><input v-model="fileActionValue" autocomplete="off" @keydown.enter.prevent="submitFileAction"/></div><small class="file-action-path">{{fileActionDialog.entry.path}}</small></main>
...
@media(max-width:700px){.creation-mode{grid-template-columns:1fr}.wizard-steps{grid-template-columns:repeat(4,1fr)}.server-blocker{align-items:flex-start;flex-wrap:wrap}.server-blocker button{margin-left:26px}.provision-card>footer{align-items:flex-start;flex-direction:column;gap:9px}.provision-card>footer>span{flex-wrap:wrap}}
