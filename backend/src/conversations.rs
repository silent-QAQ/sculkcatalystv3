...
impl From<&Conversation> for ConversationSummary {
    fn from(conversation: &Conversation) -> Self {
        Self {
            ...
            reasoning_effort: conversation.reasoning_effort.clone(),
...
async fn update_conversation_execution(
    ...
    crate::ai::validate_model_reasoning_effort(request.reasoning_effort.as_deref())
        .map_err(|error| (StatusCode::BAD_REQUEST, error))?;
    let mut data = state.inner.write().await;
    if let Some(binding) = &request.model_binding {
        crate::ai::validate_model_reasoning_effort(binding.reasoning_effort.as_deref())
            .map_err(|error| (StatusCode::BAD_REQUEST, error))?;
    ...
    conversation.reasoning_effort = request.reasoning_effort;
    let summary: ConversationSummary = (&*conversation).into();
    persist(&state, &data).await.map_err(internal)?;
    Ok(Json(summary))
}
...
mod tests {
    ...
    fn conversation_without_optional_fields_deserializes() {
        ...
        assert!(conversation.reasoning_effort.is_none());
    }
    ...
    fn forked_copies_history_and_resets_flags() {
        ...
        original.reasoning_effort = Some("high".into());
        ...
        assert_eq!(copy.reasoning_effort.as_deref(), Some("high"));
    ...
    #[test]
    fn summary_exposes_persisted_reasoning_effort() {
        let mut conversation = sample("project-1");
        conversation.reasoning_effort = Some("xhigh".into());
        let value = serde_json::to_value(ConversationSummary::from(&conversation)).unwrap();
    ...
    #[test]
    fn appended_exchange_persists_the_linked_task_id() {
