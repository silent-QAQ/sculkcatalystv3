...
> 最后更新：2026-07-27（四步创建向导、智能规划服务器、多对话任务树与安全删除）  
> 项目阶段：可交互 MVP / 技术验证原型  
...
- 工作区包含控制中心、镜像资源中心、文件管理、终端、AI 自动化、玩家社区、Skills 与 MCP。
- 主要文件：
...
- `backend/src/catalog.rs` 提供核心/插件目录、项目与版本 CRUD、解析、下载重定向及 OpenAPI。
...
flowchart LR
    ...
    API --> FILES[服务器独立工作区]
    API --> JAVA[Java 子进程]
    API --> CATALOG[核心与插件资源目录]
    API --> DOWNLOADER[核心下载执行器]
    CATALOG --> DOWNLOADER
    CATALOG --> UPSTREAM[上游制品地址]
    DOWNLOADER --> UPSTREAM
    DOWNLOADER --> FILES
    UI --> CHAT[AI 对话与任务入口]
    CHAT --> API
...
| 首次开服向导 | 已实现 | 四步普通创建；名称与位置独立；智能创建仅建立规划项目与对话，不预设核心、不写文件 | Java 自动安装、远程路径执行、核心构建号选择 |
| 服务器工作区初始化 | 已实现 | 创建目录、配置、脚本、插件与日志目录 | 模板版本管理、跨平台脚本 |
| 服务器启停 | 部分实现 | Rust 按每服内存参数启动 Java、采集输出、发送 `stop` | 崩溃恢复、进程重连、CPU/系统内存限制、重启策略 |
| 服务器终端 | 已实现 | WebSocket 实时日志、命令真实写入 Java stdin、进程未运行时模拟输出（SIM 标记） | 命令历史、自动补全、多会话 |
| 文件管理 | 已实现 | 安全目录浏览、文本读取、编辑、保存、新建目录 | 上传、下载、移动、复制、删除、差异对比 |
| 镜像资源中心 | 已实现（本地） | 核心/插件目录、项目与版本 CRUD、搜索筛选、解析、307 下载与 OpenAPI | 对象存储、上传、Range、ETag、认证、分页 |
| 核心下载执行器 | 部分实现 | 目录解析优先、文件大小与 SHA-256 校验、流式进度、来源回退、取消、安全替换 `server.jar` | 断点续传、状态恢复、所有来源的可信摘要、下载限速 |
| AI 对话 | 部分实现 | OpenAI 格式提供商接入、SSE 流式输出、每服务器多对话历史持久化、情景模型绑定、快捷切换 | 工具调用、跨对话知识库、上下文压缩、token 统计 |
| AI 自动化 | 部分实现 | 风险分级、审批、取消、任务队列、三档审核模式（请求批准/替我审核/完全访问） | 实际执行器、回滚、依赖图、任务恢复 |
| 玩家管理 | 部分实现 | 玩家列表、状态、身份、余额、处罚预览 | RCON/插件实时同步、白名单、权限组 |
...
| Skills | 部分实现 | 列表、版本、来源与启停 | Skill 加载器、权限沙箱、安装与升级 |
...
- 最大内存写入服务器状态；Rust 直接启动与生成的 `start.ps1` 共用相同的 `-Xms/-Xmx` 参数，旧状态默认迁移为 8 GB。
...
- 独立的核心库与插件库，展示项目资料、维护者、标签、兼容版本、渠道、加载平台、下载量和最新版本。
...
- 成功后先保留旧核心的回滚路径，再将临时文件替换为 `server.jar`；安装失败会恢复旧核心。完成后同步目录下载计数、bootstrap/download 任务、服务器状态与日志。
- 支持取消；取消或来源失败会清理临时文件，所有来源失败后任务进入 `failed`。
- 当前不是断点续传：重新下载会从头开始，活动状态也不会跨后端重启恢复。
...
- `GET /api/v1/resolve?kind={core|plugin}&project={slug}&minecraft={version}&channel={channel}`
...
- 资源中心只保存元数据和上游 `download_url`，下载端点返回 HTTP 307；尚未接入本地对象存储、制品上传、CDN 或副本管理。
- 重定向下载不提供 Range、ETag、缓存协商或断点续传；下载计数表示稳定入口被请求，不代表上游文件一定成功传输。
- 核心下载器使用 `server.jar.part`，失败或取消后会清理临时文件，重试会从头开始；尚无暂停、恢复、限速或跨重启任务恢复。
...
4. **可靠进程管理**
   - 已完成每服最大内存与 JVM 堆参数持久化；待补每服 PID、完整启动参数和运行状态恢复。
   - 崩溃识别、自动重启和退避策略。
   - 优雅关服超时处理与强制终止回退。
...
5. **真实 AI Agent（对话链路已落地，待补工具调用）**
   ...
   - 工具权限、审批和审计（审核模式三档已落地，待接真实工具执行器）。
