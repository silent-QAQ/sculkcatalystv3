...
> 最后更新：2026-07-31（持久化首次初始化任务、恢复、取消与重试闭环）  
> 项目阶段：可交互 MVP / 技术验证原型  
...
| 服务器启停 | 部分实现 | 真实 Child actor、实例代际、就绪检测、安全停止/超时强杀、原子重启、Unix 进程组 | Windows Job Object、崩溃后进程重连、真实资源指标 |
...
| Sculk Cloud 与主机代理 | 部分实现 | 云账号、工作区模板、Windows/Linux 独立 Agent、一键生成短期 bootstrap 配置、安全配对、指纹确认、出站心跳、在线状态与撤销、完整 Shell/终端能力 | 受限文件/日志/进程操作、任务租约、审批联动、断线恢复与审计 |
...
- 资源中心保存元数据和源站 `download_url`，下载端点返回 HTTP 307；已提供独立高带宽源站部署与静态对象目录示例，尚未实现 Web 上传、Range 代理或副本管理。
