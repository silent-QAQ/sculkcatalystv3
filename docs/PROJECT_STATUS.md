...
| 首次开服向导 | 已实现 | 四步普通创建；远程资源库精确构建选择；本地 JAR 上传；Windows x64、Linux x64/ARM64 托管 Java；智能创建仅建立规划项目与对话 | 远程路径执行、完整核心兼容矩阵 |
| 服务器工作区初始化 | 已实现 | 创建目录、配置、Windows/Linux 启动脚本；资源库模式持久 `server_provision` 自动下载校验核心，本地上传模式校验 JAR 后复用同一初始化流程；支持取消、重试和后端重启后安全恢复 | 模板版本管理、字节级跨重启续传 |
...
| 玩家管理 | 部分实现 | Paper/Folia v2 实时桥接、`playerdata` 仅作断桥/离线只读兜底、搜索排序、管理资料、等级坐标、背包/末影箱及容器预览、可指定的 PlaceholderAPI 白名单字段 | 权限组、游戏数据写入、Paper/Folia 实服联调矩阵 |
...
- 打开向导时并行读取系统信息与远程资源代理 `/api/resource-catalog/api/catalog/cores`，远程不可用时回退本机 `GET /api/catalog/cores` 缓存，并继续读取指定核心的 stable 构建列表。
- Minecraft 版本和资源库构建来自所选核心的已发布元数据；创建时固定项目 slug、构建版本、文件大小和 SHA-256，不会静默替换为另一个构建。
- 可切换到本地上传模式，选择最大 256 MiB 的 `.jar`；上传后服务端校验 ZIP/JAR 与 Manifest，再安装为 `server.jar`。
- 检测本机 Java、系统架构和数据目录。
...
- 创建以下内容：
  ...
  - `plugins/`
  - `logs/`
- 资源库模式普通创建同时持久化并立即派发 `server_provision`：依次检查工作区与 EULA、解析精确构建、下载并校验 `server.jar`、检查 Java，成功后才解除启动门禁；本地上传模式先等待专用上传接口，上传成功后再派发同一任务。
...
- 玩家管理支持按游戏名、UUID、显示名、身份、标签、备注搜索，并可按玩家名、在线状态、等级和更新时间排序；展示名称、身份、标签和备注只写入 Sculk 管理状态。
- `SculkCatalystPaperBridge` 为 Java 21、Paper API `1.21.6` 编译基线的只读 Paper/Folia 插件，声明 `folia-supported: true`。在线玩家的等级、维度、坐标、背包、装备栏、副手和末影箱通过专属桥接实时读取。
- 桥接协议已升级为 v2：`hello_init -> 一次性 challenge -> 签名 hello -> 签名 hello_ack`，之后会话双向 HMAC 签名，`payload_json` 为无填充 Base64URL 编码的 JSON。密钥只留在后端受控配置和插件配置中。
- 断桥会清空在线 presence，但会保留短期只读快照并在界面标记为缓存；`world/playerdata/*.dat` 只用于离线或断桥的只读兜底，不能替代在线实时数据。
- 支持潜影盒、收纳袋及其嵌套物品预览；不会传输原始 NBT、序列化 `ItemStack` 或插件私有数据。
- 前端可指定最多 10 个 PlaceholderAPI 显示字段。PAPI 为可选依赖，插件只会解析 `config.yml` 的 `papi.fields` 白名单中精确列出的变量。
- 尚未在真实 Paper/Folia 服务端完成联调；当前构建和协议测试不构成真实服务器兼容性结论。
- 玩家警告、踢出、封禁操作预览。
- 玩家反馈分类、情绪和聚类摘要。
- 玩法投票创建与计票。
- 经济总资产和通胀指标面板。
...
- `POST /api/servers/{id}/core/upload`（创建向导本地核心 JAR 上传、校验和安装）
...
- 尚未接入 RCON、Query 或代理网络 API。玩家管理已接入只读的 `SculkCatalystPaperBridge`，但它不提供游戏数据写入、权限组管理或通用插件消息通道。
- Paper/Folia 桥接尚未在真实服务端完成联调；目标环境仍需验证插件启停、重连、玩家跨区移动、退出、容器预览和已启用的 PAPI 扩展。
...
8. RCON/Query 或性能数据接入，补充真实性能数据与更广泛的运营指标。
